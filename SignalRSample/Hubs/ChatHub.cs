...
namespace SignalRSample.Hubs
{
    public class ChatHub : Hub
    {
        ...
        public async Task SendOpenPrivateMessage(string receiverId)
        {
            var userId = Context.User.FindFirstValue(ClaimTypes.NameIdentifier);
            var userName = Context.User.FindFirstValue(ClaimTypes.Name);

            await Clients.User(receiverId).SendAsync("ReceiveOpenPrivateMessage", userId, userName);
        }
        public async Task SendDeletePrivateMessage(string chatId)
        {
            await Clients.All.SendAsync("ReceiveDeletePrivateMessage", chatId);
        }
    }
}